...
app.post('/gerar-pix', async (req, res) => {
    ...
    try {
        ...
        // ADICIONADO: Vamos imprimir os primeiros 150 caracteres para ver a resposta nua e crua
        console.log(`\n📦 Resposta bruta do servidor: ${textoBruto.substring(0, 150)}`);
        
        try {
            const data = JSON.parse(textoBruto);
            if (data.dados) {
                console.log(`✔️ Sucesso! Registros encontrados: ${data.total || '?'}`);
            } else if (data.dados_funcao && data.dados_funcao.msg) {
                console.log(`➖ Mensagem da prefeitura: ${data.dados_funcao.msg}`);
            } else {
                console.log(`❌ O servidor aceitou, mas devolveu vazio.`);
            }
            res.json({ sucesso: true, dados_da_prefeitura: data });
        } catch (parseError) {
            console.error("❌ ERRO: O servidor retornou HTML inválido.");
